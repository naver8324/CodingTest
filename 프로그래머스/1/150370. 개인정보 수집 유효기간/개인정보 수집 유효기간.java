...
class Solution {
    public int[] solution(String today, String[] terms, String[] privacies) {
        ...
        for (int i = 0; i < privacies.length; i++) {
            ...
            int[] privacyDate = Arrays.stream(privacySplit[0].split("\\."))
                                      .mapToInt(Integer::parseInt)
                                      .toArray();
            int term = termsMap.get(privacySplit[1]);
            int[] expirationDate = getExpirationDate(privacyDate, term);
            privaciesInfo[i] = expirationDate;
        }
    ...
    private int[] getExpirationDate(int[] privacyDate, int term) {
        int[] expirationDate = Arrays.copyOf(privacyDate, privacyDate.length);
        expirationDate[1] += term;
        while (expirationDate[1] > 12) {
            expirationDate[0]++;
            expirationDate[1] -= 12;
        }
